...
/*        
        pipe_offset = 0;
        pipe_remain_byte = frame_420p_len;
        
        while (pipe_remain_byte > 0) {
        	if (pipe_remain_byte > pipe_max_byte) {
        		pipe_wbyte = pipe_max_byte;
        	} else {
        		pipe_wbyte = pipe_remain_byte;
        	}
        	
        	pipe_ret_byte = emulcap_pipe_write(frame_420p->data[0] + pipe_offset, pipe_wbyte);
        	
        	pipe_offset += pipe_ret_byte;
        	pipe_remain_byte -= pipe_ret_byte;
        	
        	printf("%s: pipe_ret_byte = %d, pipe_wbyte = %d, pipe_remain_byte = %d\n", __func__, pipe_ret_byte, pipe_wbyte, pipe_remain_byte);
        }
*/
...
*/
	...
	frame_422->format = AV_PIX_FMT_YUYV422;
    frame_422->width  = codec_ctx->width;
    frame_422->height = codec_ctx->height;
    
	frame_422_len = av_image_alloc(frame_422->data, frame_422->linesize, frame_422->width, frame_422->height, frame_422->format, 1);
	printf("%s: frame_422_len = %d\n", __func__, frame_422_len);
	
	frame_420p = av_frame_alloc();
	...
	frame_420p->format = AV_PIX_FMT_YUV420P;
    frame_420p->width  = codec_ctx->width;
    frame_420p->height = codec_ctx->height;
...
*/    
	...
	while (1) {
		// rbyte = read(i_fd, buffer, size);
		rbyte = read(i_fd, frame_422->data, frame_422_len);
		...
		sws_scale(sws_422_420p, (const uint8_t* const *)frame_422->data, frame_422->linesize, 0, 
				  frame_420p->height, frame_420p->data, frame_420p->linesize);
