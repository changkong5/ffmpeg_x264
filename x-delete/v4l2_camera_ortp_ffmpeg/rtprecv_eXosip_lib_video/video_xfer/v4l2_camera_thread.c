...
#if 0
	// 垂直翻转也不起作用
    fmt.fmt.pix.field = V4L2_FIELD_INTERLACED; // 设置字段
    // 进行垂直翻转
    fmt.fmt.pix.height = -fmt.fmt.pix.height; // 负值表示垂直翻转
#endif
...
/*    
    ...
    //control.value = 0;
    //ioctl(v4l2_fd, VIDIOC_S_CTRL, &control);
 
    // 设置垂直镜像
    control.id = V4L2_CID_VFLIP;
    control.value = 1; // 1表示启用翻转，0表示禁用
    ioctl(v4l2_fd, VIDIOC_S_CTRL, &control);
 */
...
static void v4l2_read_data(void)
{
    ...
    for ( ; ; ) {
        ...
        for(buf.index = 0; buf.index < FRAMEBUFFER_COUNT; buf.index++) {
            ...
            if (count < 2) {
            	yuyv422_420p_packet_write((void *)buf_infos[buf.index].start, buf_infos[buf.index].length);
            }
            count = yuyv422_420p_packet_length();
            // yuyv422_420p_packet_write((void *)buf_infos[buf.index].start, buf_infos[buf.index].length);
            
            printf("%s: count = %d, buf.index = %d, buf_infos[buf.index].length = %ld\n", __func__, count, buf.index, buf_infos[buf.index].length);
