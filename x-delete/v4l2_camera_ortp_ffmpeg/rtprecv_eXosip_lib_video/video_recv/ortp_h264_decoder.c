	...
	while (1) {
		...
		while (rbyte > 0) {
			...
			while (1) {
        		...
        		pr_info("%s: AV_PIX_FMT_YUV420P = %d, frame->width = %d, frame->height = %d, frame->format = %d\n", __func__, AV_PIX_FMT_YUV420P, frame->width, frame->height, frame->format);
				...
				pr_info("%s: ========1======== >>>> size = %d, count = %d\n", __func__, size, count);
				if (count < 2) {
					yuv420p_argb_packet_write(buffer, size);
				}
				
				count = yuv420p_argb_packet_length();
				// p420p_argb_packet_pipe_write(buffer, size);
				pr_info("%s: ========2======== >>>> size = %d, count = %d\n", __func__, size, count);
			}
		}
	}
	pr_debug("%s: <00> rbyte = %d, len = %d, packet->size = %d\n", __func__, rbyte, len, packet->size);
