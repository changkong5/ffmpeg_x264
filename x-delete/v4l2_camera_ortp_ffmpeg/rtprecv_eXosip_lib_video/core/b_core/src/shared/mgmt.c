...
static bool can_write_data(struct io *io, void *user_data)
{
	...
	DBG(mgmt, "<mgmt->reply_queue> request = %p", request);
	if (!request) {
		...
		DBG(mgmt, "<mgmt->request_queue> request = %p", request);
		if (!request)
			return false;
...
{
	DBG(mgmt, "queue_isempty(mgmt->pending_list) = %d, queue_isempty(mgmt->reply_queue) = %d", queue_isempty(mgmt->pending_list), queue_isempty(mgmt->reply_queue));
	...
	DBG(mgmt, "mgmt->writer_active = %d", mgmt->writer_active);
...
static bool can_read_data(struct io *io, void *user_data)
{
	...
	DBG(mgmt, "event  = %#04x (%d)", event, event);
	DBG(mgmt, "index  = %#04x (%d)", index, index);
	DBG(mgmt, "length = %#04x (%d)", length, length);
...
static ssize_t io_write(struct io *io, void *buf, int len)
{
	ssize_t ret;
	struct iovec iov;
	...
	iov.iov_len = len;
	DBG(mgmt, "len = %d, buf = %p", len, (char *)buf);
...
ssize_t mgmt_send_cmd_complete(struct mgmt *mgmt, uint16_t index, uint16_t cmd, uint8_t status, uint16_t len, uint8_t *buf)
{
	uint8_t packet[64];
	uint16_t i, total, length;
	uint16_t opcode = MGMT_EV_CMD_COMPLETE;
	struct mgmt_hdr *hdr = (void *)packet;
	struct mgmt_ev_cmd_complete *cc = (void *)(packet + MGMT_HDR_SIZE);
	
	memset(packet, 0, sizeof(packet));
	
	length = sizeof(struct mgmt_ev_cmd_complete) + len;
	...
	total = MGMT_HDR_SIZE + length;
	DBG(mgmt, "total = %d", total);
	
	for (i = 0; i < total; i++) {
		DBG(mgmt, "packet[%d] = (0x%02x) %d", i, packet[i], packet[i]);
	}
	
	return io_write(mgmt->io, packet, total);
}

ssize_t mgmt_send_cmd_status(struct mgmt *mgmt, uint16_t index, uint16_t cmd, uint8_t status)
{
	uint8_t packet[9];
	uint16_t i, total, length;
	uint16_t opcode = MGMT_EV_CMD_STATUS;
	struct mgmt_hdr *hdr = (void *)packet;
	struct mgmt_ev_cmd_status *cs = (void *)(packet + MGMT_HDR_SIZE);
	
	memset(packet, 0, sizeof(packet));
	
	length = sizeof(struct mgmt_ev_cmd_complete);
