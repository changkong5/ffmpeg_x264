				...
				DBusMessage *message, void *user_data)
{
	...
	printf("1> %s: data->introspect = %p, data->introspect  = %s\n", __func__, data->introspect, data->introspect);
	...
	printf("2> %s: data->introspect = %p, data->introspect  = %s\n", __func__, data->introspect, data->introspect);
							...
							DBusMessage *message)
{
	...
	printf("1> %s: sig = %p, sig = %s, *sig = 0x%02x\n", __func__, sig, sig, *sig);

	for (; args && args->signature && *sig; args++) {
		p = args->signature;
		
		printf("1-1>%s: sig = %s, p = %p, p = %s\n", __func__, sig, p, p);

		for (; *sig && *p; sig++, p++) {
			printf("1-1-1>%s: sig = %s, p = %p, p = %s, *p = 0x%02x, *sig = 0x%02x,\n", __func__, sig, p, p, *p, *sig);
			if (*p != *sig)
				return FALSE;
		}
		
		printf("1-2>%s: sig = %s, p = %p, p = %s\n", __func__, sig, p, p);
	}
	
	printf("2> %s: p = %p, sig = %s\n", __func__, p, sig);
...
static gboolean process_changes(gpointer user_data)
{
	...
	remove_pending(data);
	printf("%s: ----9-8---data->added = %p\n", __func__, data->added);
	if (data->added != NULL)
		emit_interfaces_added(data);

	printf("%s: ----9-8---data->pending_prop = %d\n", __func__, data->pending_prop);
	/* Flush pending properties */
	if (data->pending_prop == TRUE)
		process_property_changes(data);

	printf("%s: ----9-8---data->removed = %p\n", __func__, data->removed);
...
static void generic_unregister(DBusConnection *connection, void *user_data)
{
	...
	if (data->process_id > 0) {
		g_source_remove(data->process_id);
		data->process_id = 0;printf("%s: ----9-7-2--\n", __func__);
					...
					DBusMessage *message, void *user_data)
{
	...
	printf("1> %s: interface = %s\n", __func__, interface);

	iface = find_interface(data->interfaces, interface);
	if (iface == NULL)
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
		
	printf("2> %s: interface = %s\n", __func__, interface);

	for (method = iface->methods; method &&
			method->name && method->function; method++) {

		printf("3> %s: iface->name = %s, method->name = %s\n", __func__, iface->name, method->name);

		if (dbus_message_is_method_call(message, iface->name,
							method->name) == FALSE)
			continue;
			
		printf("3-1> %s: iface->name = %s, method->name = %s\n", __func__, iface->name, method->name);

		if (check_experimental(method->flags,
					G_DBUS_METHOD_FLAG_EXPERIMENTAL))
			return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;

		printf("3-2> %s: iface->name = %s, method->name = %s\n", __func__, iface->name, method->name);

		if (g_dbus_args_have_signature(method->in_args,
							message) == FALSE)
			continue;
			
		printf("3-3> %s: iface->name = %s, method->name = %s\n", __func__, iface->name, method->name);	

		if (check_privilege(connection, message, method,
						iface->user_data) == TRUE)
			return DBUS_HANDLER_RESULT_HANDLED;

		printf("4> %s: iface->name = %s, method->name = %s\n", __func__, iface->name, method->name);
