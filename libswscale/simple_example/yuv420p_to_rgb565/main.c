...
-v loglevel 设置日志级别
-report 生成一个报告
-max_alloc bytes 设置单个块可以分配的最大大小
-sources device 列出输入设置源
-sinks device 列出输出设备的接收器
-x width 设置显示的宽
-y height 设置显示的高
-s size 设置帧的大小 即宽x高WxH
-fs 全屏
-an 禁用音频
-vn 禁用视频
-sn 禁用字幕
-t duration 设置播放音频或者视频的次数
-byte val 按照bytes跳转 0关闭，1开启，-1自动
-seek_interval seconds 设置按下left/right跳转的单位
-nodisp 禁用图形显示
-noboreder 无边框的界面
-alwaysontop 设置界面在上方
-volume volume 设置音量从0到100
-f fmt 指定解码改革是
-window_title window title 设置界面名字
-af filter_graph 设置音频过滤
-showmode mode 设置显示方式(0 = video, 1 = waves, 2 = RDFT)
-i input_file 读取指定 文件
-codec decoder_name 指定解码器
-autorotate 自动旋转视频
*/

// lsb@lsb:yuv420p_to_rgb565$ ffplay -pix_fmts
// IO... rgb565be               3            16
// IO... rgb565le               3            16

// ffplay -pixel_format rgb565le -s 352x288 yuv420p_w352xh288.rgb
...
int main(int argc, char* argv[])
{
	...
	// enum AVPixelFormat dst_pixfmt=AV_PIX_FMT_RGB565BE;
	
	enum AVPixelFormat dst_pixfmt=AV_PIX_FMT_RGB565LE;
